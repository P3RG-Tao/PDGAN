...
# dir = '... your path/faces/'
dir = 'D:/kyw/Palmprint_Recognition-master/Palmprint'
# path = []
#
...
def train():
    for i, (image,_) in tqdm.tqdm(enumerate(dataloader)):       # type((image,_)) = <class 'list'>, len((image,_)) = 2 * 256 * 3 * 96 * 96
        ...
        if (i + 1) % d_every == 0:
            ...
            output = torch.randn(50, 100)
            # 调整维度
            new_tensor = torch.unsqueeze(output, dim=2)
            new_tensor = torch.unsqueeze(new_tensor, dim=3)

            # # 目标大小和形状
            # target_size = (50, 100, 1, 1)
            # # 计算额外的元素数量
            # extra_elements = torch.prod(torch.tensor(target_size)) - torch.prod(output)
            # # 生成额外的随机值，以满足额外的元素数量
            # extra_values = torch.randn(int(extra_elements))
            # # 将额外的值重塑成目标形状
            # extra_values = extra_values.view(target_size)
            # # 合并原始张量和额外的值，得到目标大小和形状的张量
            # final_tensor = torch.stack((output, extra_values))
            # 调整维度
...
def test():
    net = NetGenerator()
    net.cuda()
    net.load_state_dict(torch.load("./save/gan_/generator.pth"))
    net.eval()
    ...
    plt.subplots_adjust(left=None,  # the left side of the subplots of the figure
                        ...
                        hspace=0.05)  # the amount of height reserved for white space between subplots)
    plt.suptitle('第%d迭代结果' % num, y=0.91, fontsize=15)
    plt.savefig("images/%d__cgan.png" % num)
